...
class os_t:
	def __init__ (self, cpu, memory, terminal):
		...
		self.fake_page = []
		self.page_size = 256
		numero_blocos = math.floor(self.memory.get_size()/self.page_size)
		for i in range(0,int(numero_blocos)):
			self.fake_page.append(None)
		...
		self.terminal.app_print("this is the app\n")
	...
	def syscall (self):
		...
		# 1 is print string service
			...
			while True:
				...
				self.terminal.app_print(str(chr(text)))
				endereco_fisico = endereco_fisico + 1
				text = self.memory.read(endereco_fisico)
				
				
		
		# 2 is newline print service
		elif service == 2:
			self.terminal.app_print("\n")
			
			 	
	
		# 3 is integer print service
		elif service == 3:
